...
import DataTable from "../components/DataTable"; // Table reusable
...
export default function PostPage() {
  const { data, isLoading, error } = useQuery<{ result: PostModel[] }>({
    queryKey: ["users"],
    queryFn: fetchPosts,
  });
  ...
  return (
    <div className="container mx-auto py-10">
      <div className="flex justify-between items-center mb-5">
        <h2 className="text-2xl font-bold text-[#1A120B]">List Posts</h2>
        <Link href={`/post/create`}>
          <Button className="bg-green-500 text-white">Create Post</Button>
        </Link>
      </div>
